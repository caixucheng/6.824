...
func TestReElection2A(t *testing.T) {
	...
	log.Printf("The leader %v will disconnect", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	log.Printf("The leader %v will re-join", leader1)
	cfg.connect(leader1)
	...
	// be elected.
	log.Printf("The leader %v will disconnect", leader1)
	log.Printf("The %v server will disconnect", (leader2+1)%servers)
	...
	log.Printf("The %v server will re-join", (leader2+1)%servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	log.Printf("The leader %v will re-join", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
